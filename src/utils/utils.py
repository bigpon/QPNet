...
def read_hdf5(hdf5_name, hdf5_path):
    ...
    if hdf5_path not in hdf5_file:
        ...
        print("Please check the data path in hdf5 file('%s')." % hdf5_name)
        sys.exit(-1)
...
def shape_hdf5(hdf5_name, hdf5_path):
    ...
    if check_hdf5(hdf5_name, hdf5_path):
        ...
        print("There is no such a file or dataset. (%s, %s)" % (hdf5_name, hdf5_path))
        sys.exit(-1)
...
def check_filenames(filename_list):
    filename_list = [os.path.basename(x).split('.')[0] for x in filename_list]
    return len(set(filename_list))==1
